...
async function runESLint(): Promise<LintResult> {
  ...
  try {
    const { stdout, stderr } = await runCommand('npx eslint --ext .ts,.tsx,.js,.jsx --fix .');
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await fs.writeFile(biomeConfigPath, JSON.stringify(defaultConfig, null, 2));
        configPath = biomeConfigPath;
      }
    }
    ...
    const formatResult = await runCommand(`npx @biomejs/biome format --write ${configFlag} .`);

    // Then run Biome lint with auto-fix
    const lintResult = await runCommand(`npx @biomejs/biome lint --apply ${configFlag} .`);
...
async function runPrettier(): Promise<LintResult> {
  ...
  try {
    ...
    const prettierConfigPath = path.join(process.cwd(), 'config', '.prettierrc.json');
...
async function lintAndFix() {
  try {
    ...
    for (const result of results) {
      console.log(`${result.tool}: ${result.success ? '✅ Success' : '❌ Issues found'}`);
      if (result.error) {
