...
type DragAction =
  | { type: 'START_DRAG'; payload: { item: any; id: string; containerId: string; position: Position } }
...
interface DragContextValue {
  dragState: DragState;
  startDrag: (item: any, id: string, containerId: string, event: React.MouseEvent | React.TouchEvent) => void;
...
export const DragProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const registerDropContainer = useCallback((id: string, element: HTMLElement, accepts: string[]) => {
  ...
  // End drag operation
  const endDrag = useCallback((targetContainerId?: string, targetIndex?: number): DragResult | null => {
    if (!dragState.isDragging || !dragState.draggedItem || !dragState.draggedId || !dragState.sourceContainerId) {
      return null;
    }
