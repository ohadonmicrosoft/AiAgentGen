...
export function usePerformance({
  ...
  useEffect(() => {
    ...
    const removeListener = performanceMonitor.addListener((metric) => {
      ...
      if (metric.name.startsWith(`component:${name}:`) || 
...
export function useOperationPerformance(componentName: string, operationName: string) {
  ...
  const end = useCallback((metadata?: Record<string, any>) => {
    const duration = performance.now() - startTimeRef.current;
    performanceMonitor.endMarker(`component:${componentName}:operation:${operationName}`, metadata);
    return duration;
  }, [componentName, operationName]);
