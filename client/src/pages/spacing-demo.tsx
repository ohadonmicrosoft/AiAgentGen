      ...
      >
        <div>
          ...
          <p className="fluid-body text-muted-foreground mt-2 max-w-3xl">
            Explore the dynamic spacing system that automatically adjusts spacing based on viewport size.
            This creates more harmonious layouts that maintain visual hierarchy across all device sizes.
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Elements with fluid padding utilities
                    </CardDescription>
                  </CardHeader>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Elements with fluid margin utilities
                    </CardDescription>
                  </CardHeader>
            ...
            <section>
              ...
              <Tabs defaultValue="xs">
                ...
                <TabsContent value="xs" className="mt-4">
                  <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 ${fluidSpacingToClassName({ gap: 'xs' })}`}>
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div key={i} className="aspect-square bg-primary/10 border border-primary/20 rounded-md flex items-center justify-center">
                ...
                <TabsContent value="sm" className="mt-4">
                  <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 ${fluidSpacingToClassName({ gap: 'sm' })}`}>
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div key={i} className="aspect-square bg-primary/10 border border-primary/20 rounded-md flex items-center justify-center">
                        <span className="text-xs">Item {i+1}</span>
                      </div>
                ...
                <TabsContent value="md" className="mt-4">
                  <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 ${fluidSpacingToClassName({ gap: 'md' })}`}>
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div key={i} className="aspect-square bg-primary/10 border border-primary/20 rounded-md flex items-center justify-center">
                        <span className="text-xs">Item {i+1}</span>
                      </div>
                ...
                <TabsContent value="lg" className="mt-4">
                  <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 ${fluidSpacingToClassName({ gap: 'lg' })}`}>
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div key={i} className="aspect-square bg-primary/10 border border-primary/20 rounded-md flex items-center justify-center">
                        <span className="text-xs">Item {i+1}</span>
                      </div>
                ...
                <TabsContent value="xl" className="mt-4">
                  <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 ${fluidSpacingToClassName({ gap: 'xl' })}`}>
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div key={i} className="aspect-square bg-primary/10 border border-primary/20 rounded-md flex items-center justify-center">
                        <span className="text-xs">Item {i+1}</span>
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      More spacing for brief content
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Reduced spacing for longer content
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="bg-muted rounded-lg p-4">
                      <p className="text-sm">
                        This is a much longer paragraph that contains more information. 
                        When content is longer, we can reduce the spacing to make better
                        use of the available space and maintain a good content density.
                        This helps create a more balanced visual hierarchy and improves
                        readability for longer blocks of text.
                      </p>
                    </div>
                  </CardContent>
                </Card>
